...
function PaymentModal({ isOpen, onClose, onConfirmPayment, paymentData }) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setLoading(false);
      setMethod('card');
      setCardNumber(''); setExpiry(''); setCvv(''); setName(''); setPhone('');
      setReceiptFile(null);
    }
  }, [isOpen]);
  ...
  const handlePay = async (e) => {
    e.preventDefault();
    if (method === 'card' && (cardNumber.length < 19 || !expiry || !cvv || !name)) return alert("Datos de tarjeta incompletos");
    if (method === 'yape' && phone.length < 9) return alert("Celular inválido");
            ...
            <>
              ...
              <form onSubmit={handlePay} className="space-y-4 animate-fade-in-up">
                ...
                <div className="pt-2 flex gap-3">
                    ...
                    <button
                        type="submit"
                        className={`flex-[2] py-2.5 text-white font-bold rounded-lg shadow-md transform active:scale-95 transition-all flex justify-center items-center gap-2 text-sm ${
                            method === 'yape' ? 'bg-purple-600 hover:bg-purple-700' :
